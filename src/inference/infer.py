...
class TorchInference:
    ...
    def predict_from_image(self, pil_image: Image.Image) -> Dict[str, Any]:
        ...
        eth_label = (
            ETHNICITY_LABELS[eth_idx]
            if eth_idx < len(ETHNICITY_LABELS)
            else f"cls_{eth_idx}"
        )
...
class ONNXInference:
    ...
    def __init__(self, onnx_path: str, provider_preference: Optional[list] = None):
        ...
        if not _HAS_ONNXRUNTIME:
            raise RuntimeError(
                "onnxruntime not installed. Install onnxruntime-gpu or onnxruntime."
            )
...
def get_best_inference(
    ...
    inf = TorchInference(
        checkpoint_path=checkpoint_path, device=device, use_fp16=use_fp16
    )
    return inf, backend
